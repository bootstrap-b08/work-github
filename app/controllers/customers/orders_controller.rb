    ...
    if @order.save
        current_customer.cart_items.each do |cart_item|
          @order = OrderItem.new(
            ...
            order_price: cart_item.item.price*price_with_tax,
            order_id: @order.id
          )
          @order_items.save!
          end
        Address.create!(customer_id: current_customer.id, postal_code: @order.post_code, address: @order.shipping_address, name: @order.delivery_name)
